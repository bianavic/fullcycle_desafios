...
func getWeather(c *gin.Context, localidade string) (float64, float64, float64, error) {
	tracer := otel.Tracer("service-b-tracer")
	_, span := tracer.Start(c.Request.Context(), "getWeather")
	defer span.End()
...
func getLocation(c *gin.Context, cep string) (string, error) {
	tracer := otel.Tracer("service-b-tracer")
	_, span := tracer.Start(c.Request.Context(), "getLocation")
	defer span.End()
...
func weatherHandler(c *gin.Context) {
	...
	localidade, err := getLocation(c, cep)
	if err != nil {
		c.JSON(http.StatusNotFound, gin.H{"message": "cannot find zipcode"})
		return
	}

	tempC, tempF, tempK, err := getWeather(c, localidade)
	if err != nil {
...
func main() {
	...
	r.GET("/temperatura", weatherHandler)
	r.Run(":8081")
}
